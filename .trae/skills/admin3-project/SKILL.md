...
项目根目录提供了 `build.ps1` 脚本，自动完成以下步骤：
...
脚本功能：
1. **构建前端** - 自动执行 `vite build`（先清理旧 dist）
2. **停止 Java 进程** - 自动终止占用 JAR 文件的 Java 进程，避免 `spring-boot-maven-plugin:repackage` 的 `Unable to rename` 错误
3. **Maven 打包** - 执行 `mvn clean package -pl admin3-server -am -DskipTests`

可选参数：
```powershell
.\build.ps1 -SkipFrontend   # 跳过前端构建（dist 已存在时）
.\build.ps1 -SkipClean      # 跳过 clean（增量编译，加快速度）
```
...
> **为什么需要这一步**：`spring-boot-maven-plugin:repackage` 需要将 `admin3-server-0.0.1-SNAPSHOT.jar` 重命名为 `.original`，如果 JAR 被 Java 进程占用会失败。必须先停掉所有 Java 进程。

**步骤 3：Maven 打包**
```powershell
.\mvnw.cmd clean package -pl admin3-server -am -DskipTests
```
...
构建成功后检查 JAR 包大小，**正常应约 70MB**（包含前端资源）：

```powershell
ls admin3-server/target/admin3-server-0.0.1-SNAPSHOT.jar
```

如果只有几百 KB，说明前端资源未被打包进去，需要重新执行完整构建流程。
